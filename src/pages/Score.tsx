...
const Score: React.FC = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem('userId')) {
      ...
      setUserId(storedId)
      getScore(storedId).then(({ scores: newScores, updateTime: newUpdateTime }) => {
        setScores(newScores)
        setUpdateTime(newUpdateTime)
      })
    }
                ...
                <div className="flex flex-wrap flex-1 gap-x-2 gap-y-1 min-w-0">
                  {levels.map((level) => (
                    <p
                      ...
                      className={`text-amber-950 px-2 py-0.5 rounded cursor-pointer transition-colors hover:bg-amber-400/50 ${
                        selectedLevel === level ? 'text-border !bg-amber-400 text-white' : ''
                      }`}
                    >
                      {level}
                    </p>
                ...
                <div className="flex flex-wrap flex-1 gap-x-2 gap-y-1 min-w-0">
                  {crowns.map((crown) => (
                    <p
                      ...
                      className={`text-amber-950 px-2 py-0.5 rounded cursor-pointer transition-colors hover:bg-amber-400/50 ${
                        selectedCrown === crown ? 'text-border !bg-amber-400 text-white' : ''
                      }`}
                    >
                      {crown}
                    </p>
                ...
                <div className="flex flex-wrap flex-1 gap-x-2 gap-y-1 min-w-0">
                  {ranks.map((rank) => (
                    <p
                      ...
                      className={`text-amber-950 px-2 py-0.5 rounded cursor-pointer transition-colors hover:bg-amber-400/50 ${
                        selectedRank === rank ? 'text-border !bg-amber-400 text-white' : ''
                      }`}
                    >
                      {rank}
                    </p>
                ...
                <div className="flex flex-wrap flex-1 gap-x-2 gap-y-1 min-w-0">
                  {sorts.map((sort) => (
                    <div
                      ...
                      className={`text-amber-950 px-2 py-0.5 rounded cursor-pointer flex items-center hover:bg-amber-400/50 ${
            ...
            {filteredScores.map((score: any, index: number) => (
              ...
              >
                ...
                <div className="flex space-x-4 items-center justify-end w-75 rounded-xl mx-auto bg-black/5 p-2 md:(mx-0 p-0 bg-transparent)">
                  ...
                  <div
                    className={`relative w-15 h-10 rounded-lg border-2 border-white ${
                      score.level === 1 ? 'bg-red-300' :
